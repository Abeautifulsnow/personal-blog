...
class ChangePwdForm(forms.Form):
    ...
    修改密码
    """
    old_pwd = forms.CharField(required=True, min_length=6, label='旧的密码',
                               widget=forms.PasswordInput(
                                   attrs={'class': 'form-control', 'placeholder': '请输入旧的密码'}))
    new_pwd1 = forms.CharField(required=True, min_length=6, label='新的密码',
                               widget=forms.PasswordInput(
                                   attrs={'class': 'form-control', 'placeholder': '请输入新的密码'}))
    new_pwd2 = forms.CharField(required=True, min_length=6, label='新的密码',
                               widget=forms.PasswordInput(
    ...
    def clean(self):
        ...
        if new_pwd1 != new_pwd2:
            raise forms.ValidationError('两次输入的密码不一致且密码不能为空')
        elif new_pwd1 == '':
            raise forms.ValidationError('密码不能设置为空')
        return self.cleaned_data
...
class ForgotPasswordForm(forms.Form):
    ...
    new_pwd1 = forms.CharField(min_length=6, 
                               label='新的密码',
                               widget=forms.PasswordInput(
                                   attrs={'class': 'form-control', 'placeholder': '请输入新的密码'}))
